...
async def websocket_audio_streaming(websocket: WebSocket):
    ...
    session_history = []
    last_processed_transcript = ""  # Track last processed transcript to avoid duplicates
    ...
    def on_turn(self: Type[StreamingClient], event: TurnEvent):
        nonlocal session_history, last_processed_transcript
        transcript_text = event.transcript.strip()  # Remove whitespace
        
        # Only print and process when turn ends (turn detection)
        if event.end_of_turn and transcript_text:  # Check if transcript is not empty
            # Check for duplicate processing (case-insensitive and punctuation-insensitive)
            normalized_current = transcript_text.lower().replace('?', '').replace('.', '').replace('!', '').strip()
            normalized_last = last_processed_transcript.lower().replace('?', '').replace('.', '').replace('!', '').strip()
            
            if normalized_current == normalized_last and normalized_current != "":
            ...
            print(f"TRANSCRIPTION: {transcript_text}")  # Print final transcript only
            ...
            try:
                ...
                # Process LLM streaming response synchronously (since we can't use asyncio.create_task here)
                try:
                    ...
                    logging.error(f"Error processing LLM streaming response: {e}")
